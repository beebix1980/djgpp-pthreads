...
# remove newlines
string(REPLACE "\n" "" PTHREAD_C_FLAGS_2 ${PTHREAD_C_FLAGS_2})

set(PTHREAD_STAGE3_BYPRODUCTS
	"${STAGE3_BINARY_DIR}/${CMAKE_INSTALL_PREFIX}/${DJGPP_TARGET_TRIPLET}/lib/libpthread.a")

ExternalProject_Add(PTHREAD_Stage3 DEPENDS PTHSOCK_Stage3_Ninja
	...
	# Download, verify and extract GNU Pth source archive.
	DOWNLOAD_COMMAND
		...
		${CMAKE_COMMAND}
			"-DFILENAME=<SOURCE_DIR><SOURCE_SUBDIR>/config.guess"
			"-DURL=${CONFIG_GUESS_URL}"
			"-P ${PROJECT_SOURCE_DIR}/cmake/DownloadFile.cmake" &&
		${CMAKE_COMMAND}
			"-DFILENAME=<SOURCE_DIR><SOURCE_SUBDIR>/config.sub"
			"-DURL=${CONFIG_SUB_URL}"
			"-P ${PROJECT_SOURCE_DIR}/cmake/DownloadFile.cmake" &&
	...
	CONFIGURE_COMMAND
		...
		${CMAKE_COMMAND} -E cmake_echo_color --green
			"Enter the Ninja: ${STAGE2_BYPRODUCTS} ${PTHSOCK_STAGE3_BYPRODUCTS}" &&
		...
		${SH_EXE} -c "\
			'<SOURCE_DIR><SOURCE_SUBDIR>/config.guess' | '${XARGS_EXE}' -I {} \
				'<SOURCE_DIR><SOURCE_SUBDIR>/configure' \
					--prefix='${CMAKE_INSTALL_PREFIX}/${DJGPP_TARGET_TRIPLET}' \
					--build='{}' \
		...
		${SED_EXE} -i
			"s|\\(CFLAGS.*=\\).*|\\1 -I<SOURCE_DIR><SOURCE_SUBDIR> -D_REENTRANT -mtune=${DJGPP_MTUNE} -march=${DJGPP_MARCH} ${PTHREAD_C_FLAGS_2}|"
			"Makefile"
	...
	INSTALL_COMMAND
		${CMAKE_COMMAND} -E env "PATH=${STAGE2_BINARY_DIR}/bin:$ENV{PATH}" --
		${MAKE_EXE} "DESTDIR=${STAGE3_BINARY_DIR}" install

	BUILD_BYPRODUCTS
		"${PTHREAD_STAGE3_BYPRODUCTS}"
)

add_custom_target(PTHREAD_Stage3_Ninja DEPENDS "${PTHREAD_STAGE3_BYPRODUCTS}")
add_dependencies(PTHREAD_Stage3_Ninja PTHSOCK_Stage3_Ninja PTHREAD_Stage3)
