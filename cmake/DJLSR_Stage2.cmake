...
# Stage 2 / Package 1 : Patched DJGPP runtime (based on DJLSR205)

include(GCC_Stage1)

# C_FLAGS used when compiling the DJGPP runtime (and WATT32).
# -fcommon is required
set(DJGPP_LIBC_OPTIMIZE_1 "
...
# remove newlines
string(REPLACE "\n" "" DJGPP_LIBC_OPTIMIZE_1 ${DJGPP_LIBC_OPTIMIZE_1})

set(DJLSR_STAGE2_BYPRODUCTS
	"${STAGE2_BINARY_DIR}/${DJGPP_TARGET_TRIPLET}/lib/libc.a")

if(HAVE_WATT32)
	# Because the WATT32 header files must be integrated into the DJGPP include
	# directory we shall set that up here. Compilation of WATT32 is done in the
	# next build step (where it replaces the dummy PTHSOCK_Stage2 target).
	#
	# The variables defined here are added to the DOWNLOAD_COMMAND and
	# PATCH_COMMAND of the DJLSR_Stage2 target when HAVE_WATT32 is true.

	# Downloads, verifies and extracts WATT32 source tarball (cvt text files).
	set(watt32_djlsr_download
		COMMAND ${CMAKE_COMMAND}
			"-DFILENAME=<DOWNLOAD_DIR>/${WATT32_ZIP}"
			"-DHASH=${WATT32_HASH}"
			"-DDESTDIR=<SOURCE_DIR>/watt32"
			"-DARGS=-a -o"
			"-DURLS=${WATT32_URL}"
			"-P ${PROJECT_SOURCE_DIR}/cmake/DownloadZIP.cmake"
	)

	# Apply our patches to the WATT32 header files (after converting line-endings)
	# and then combine them with the DJGPP headers. Also copy the pre-generated
	# syserr.c into place and modify the target triplet and CFLAGS in makefile.all
	# for later. Afterwards update the manifest files so that the modification
	# to the headers are included in the generated DJGPP distro files.
	# (Fixme: Create a target that rebuilds syserr.c using dosbox or something.)
	set(watt32_djlsr_patch_stage2
		...
		${SED_EXE} -i "s/\\(-Wno-strict-aliasing\\)/${DJGPP_LIBC_OPTIMIZE_1} \\1/g" watt32/src/makefile.all &&
		${SH_EXE} -c
			...
			if ! [ -f '${STAGE1_BINARY_DIR}/${DJGPP_TARGET_TRIPLET}/include/sys/djcdefs.h' ] $<SEMICOLON> then \
				rm -vf '${STAGE1_BINARY_DIR}/${DJGPP_TARGET_TRIPLET}/include/netinet/in.h' && \
				rm -vf '${STAGE1_BINARY_DIR}/${DJGPP_TARGET_TRIPLET}/include/sys/errno.h' && \
				mv -vf '${STAGE1_BINARY_DIR}/${DJGPP_TARGET_TRIPLET}/include/sys/cdefs.h' '${STAGE1_BINARY_DIR}/${DJGPP_TARGET_TRIPLET}/include/sys/djcdefs.h' && \
				mv -vf '${STAGE1_BINARY_DIR}/${DJGPP_TARGET_TRIPLET}/include/sys/ioctl.h' '${STAGE1_BINARY_DIR}/${DJGPP_TARGET_TRIPLET}/include/sys/djioctl.h' && \
				mv -vf '${STAGE1_BINARY_DIR}/${DJGPP_TARGET_TRIPLET}/include/sys/param.h' '${STAGE1_BINARY_DIR}/${DJGPP_TARGET_TRIPLET}/include/sys/djparam.h' && \
				cp -vfR watt32/inc/* '${STAGE1_BINARY_DIR}/${DJGPP_TARGET_TRIPLET}/include' $<SEMICOLON>\
			fi" &&
...
# This is the first instance that the DJGPP runtime is built, it will be
# used to compile the stage 2 toolchain and libraries.
ExternalProject_Add(DJLSR_Stage2 DEPENDS GCC_Stage1_Ninja
	PREFIX "${STAGE2_BINARY_DIR}"
	USES_TERMINAL_DOWNLOAD TRUE
	USES_TERMINAL_PATCH TRUE
	USES_TERMINAL_BUILD TRUE
	USES_TERMINAL_INSTALL TRUE
	INSTALL_DIR "${STAGE2_BINARY_DIR}"
	...
	# Apply our patches and clean the source tree.
	PATCH_COMMAND
		...
		# Do not build the fpu emulator (when building libc with -flto),
		...
		${SED_EXE} -i "s/-O2/${DJGPP_LIBC_OPTIMIZE_1}/g" "<SOURCE_DIR>/src/makefile.cfg" &&
	...
	# (WATT32 is build later by PTHSOCK_Stage2 if selected.)
	BUILD_COMMAND
		# Build
		#${CMAKE_COMMAND} -E env "PATH=${STAGE1_BINARY_DIR}/bin:$ENV{PATH}" --
		...
		${SH_EXE} -c "\
			cd '<SOURCE_DIR>' &&\
			'${ZIP_EXE}' -q \
				-@ '${CMAKE_CURRENT_BINARY_DIR}/djcrx205-stage2.zip'\
				< 'manifest/djcrx205.mft' &&\
			'${ZIP_EXE}' -q \
				-@ '${CMAKE_CURRENT_BINARY_DIR}/djdev205-stage2.zip'\
				< 'manifest/djdev205.mft' &&\
			'${ZIP_EXE}' -q \
				-@ '${CMAKE_CURRENT_BINARY_DIR}/djlsr205-stage2.zip'\
				< 'manifest/djlsr205.mft' &&\
			'${ZIP_EXE}' -q \
				-@ '${CMAKE_CURRENT_BINARY_DIR}/djtzs205-stage2.zip'\
				< 'manifest/djtzs205.mft' &&\
			'${ZIP_EXE}' -q \
				-@ '${CMAKE_CURRENT_BINARY_DIR}/djtzn205-stage2.zip'\
				< 'manifest/djtzn205.mft'"

	# Install into the stage2 prefix.
	INSTALL_COMMAND
		...
		${UNZIP_EXE} -q
			-o "${CMAKE_CURRENT_BINARY_DIR}/djcrx205-stage2.zip"
...
add_custom_target(DJLSR_Stage2_Ninja DEPENDS "${DJLSR_STAGE2_BYPRODUCTS}")
add_dependencies(DJLSR_Stage2_Ninja GCC_Stage1_Ninja DJLSR_Stage2)
